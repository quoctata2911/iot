...
const onOff = async (value) => {
  const url = `http://localhost:5000/api/device/${uid}/${homeId}/${typeOpenDetail.value._id}/control`;
  let response = await fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  <div class="h-[calc(100vh-125px)]">
    <div class="mb-2">
      ...
      <fwb-modal v-if="isShowModal" @close="closeModal">
        ...
        <template #body>
          ...
          <fwb-select v-model="value3" :options="countries" label="chọn type" />
    ...
    >
      ...
      <fwb-modal v-if="isShowModal2" @close="isShowModal2 = false">
        <template #header>
          <div class="flex items-center text-xl">Thông tin</div>
        </template>
        <template #body>
          <!-- led: {{ led }} -->
          <!-- <fwb-input v-model="value1" label="sensor " size="sm" /> -->
          <div class="text-5xl justify-center">
            <div
              v-if="typeOpenDetail.device_status.type === 'led'"
            ...
            >
              ...
              <div class="flex gap-2 mx-auto">
                ...
                <fwb-button @click="onOff(true)" gradient="red" class="w-[70px]"
                  >Tắt</fwb-button
                >
              </div>
              ...
              Trạng thái:
              {{
                (led?.device_status?.type || "chưa nhận dữ liệu") +
                " " +
                (led?.device_status?.value || "")
              }}
            </div>

            <div class="h-5"></div>
            <div
              v-if="typeOpenDetail.device_status.type === 'sensor'"
              class="text-center"
            >
              <!-- dht: {{ dht }} -->
              <div
                class="mb-4 flex gap-1 items-center text-center justify-center"
              >
                Độ ẩm:<svg
                  width="20px"
                  height="20px"
                ...
                {{
                  (dht && Number(dht.device_status.value[0]).toFixed(0)) || 0
                }}
                %.
              </div>
              <div
              ...
              >
                ...
                {{
                  (dht && Number(dht.device_status.value[1]).toFixed(0)) || 0
                }}
                °C
              </div>
            </div>
